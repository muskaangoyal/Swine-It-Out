...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** REPLACE THIS LINE ***"
    score,pig_out,num_ones = 0,False,0
    while num_rolls > 0:
        your_roll = dice()
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if player==0:
            this_turn_score=take_turn(strategy0(score0,score1),score1,dice)
            score0+=this_turn_score
        else:
            this_turn_score=take_turn(strategy1(score1,score0),score0,dice)
...
def both(f, g):
    ...
    # BEGIN PROBLEM 6
    "*** REPLACE THIS LINE ***"
    def new_commentary(score0,score1):
        first= f(score0,score1)
        second= g(score0,score1)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    "*** REPLACE THIS LINE ***"
    def highest_number(score0,score1):
        nonlocal who, previous_high, previous_score
        if who==0:
            who_score=score0
        else:
            who_score=score1
        current_gain=who_score-previous_score
        previous_score=who_score
        if current_gain>previous_high:
            previous_high=current_gain
            print(str(current_gain) + " point" + ("s" if current_gain!=1 else "") + "! That's the biggest gain yet for Player " + str(who))
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    # BEGIN PROBLEM 11
    "*** REPLACE THIS LINE ***"
    play_score= score+free_bacon(opponent_score)
    if is_swap(play_score,opponent_score) or free_bacon(opponent_score) >=margin:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
